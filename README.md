...
1. Reviews open positions — closes any that have hit exit criteria
...
Between cycles it waits at an idle prompt (15 min default). You can ask it questions, type `report` to reprint the last cycle summary, press Enter to run immediately, or just wait.

At 15:45 ET it liquidates all open positions and waits for the next session.
...
agent/
  runner.py            — Ollama tool-calling loop (max 25 calls, guards, decision capture)
  tools.py             — tool definitions + implementations + K-4SH system prompt
  executor.py          — buy guard rails, fractional share sizing, wiki fallback
  report.py            — cycle report renderer (shown after every cycle)
...
wiki/                  — K-4SH's persistent memory (gitignored — starts fresh per install)
...
tests/                 — unit tests covering flavor, report rendering, wiki P&L
```
...
- **8% of portfolio value** per trade (fractional shares supported)
...
- Once-only tool guards: portfolio state, market conditions, and wiki writes are capped at one call per cycle to prevent looping
- Duplicate search dedup: identical web search queries are blocked within a cycle
- Hard block: cannot `close_position` on a ticker not currently held
...
- Intraday gain exceeds +2% (profit-taking)
...
**EOD liquidation** — at 15:45 ET, all positions are market-sold automatically regardless of P&L.
...
```
╔══════════════════════════════════════════════════════════════════╗
║                 CYCLE REPORT — 2026-04-24 14:00 ET               ║
╠══════════════════════════════════════════════════════════════════╣
║  DECISION: BUY 19 MSFT @ $419.65                                 ║
║  "Executing the Kessel Run. Punch it."                           ║
╠══════════════════════════════════════════════════════════════════╣
║  PORTFOLIO                                                       ║
║    ├─ MSFT   19 shares  $419.65   +$0.00  (+0.00%)               ║
║    ├─ AAPL   29 shares  $270.00   +$0.00  (+0.00%)               ║
║    └─ Cash   $24,795.23                                          ║
║                                                                  ║
║  Total Value:      $   98,500.00                                 ║
║  Unrealized P&L:   +$0.00                (still at risk)         ║
║  Realized P&L:     +$0.00                (locked in)             ║
╠══════════════════════════════════════════════════════════════════╣
║  [MAC] Upgrade:  0.00 / 800 credits  [░░░░░░░░░░]  0%            ║
║  "Every closed position brings the upgrade one step nearer."     ║
╠══════════════════════════════════════════════════════════════════╣
║  Next scan in 15 min  |  Ask me something or press Enter         ║
╚══════════════════════════════════════════════════════════════════╝
```
...
The wiki is gitignored and starts fresh on each install. It is local state, not part of the repository.
